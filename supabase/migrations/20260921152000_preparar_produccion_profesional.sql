-- Preparación profesional e idempotente de producción desde un pedido.
-- Una sola transacción: OP + operaciones de ruta + piezas requeridas.
...
begin
  ...
  -- La restricción UNIQUE(pedido_id) también protege contra doble OP concurrente.
  select *
    into v_op
  ...
  -- La ruta del pedido es la fuente de verdad. No se crean pasos comerciales
  -- ni de cierre como operaciones de fabricación.
  ...
  -- Compatibilidad con pedidos antiguos sin ruta: el taller es la operación
  -- mínima de fabricación y no se inventa una ruta comercial completa.
  if coalesce(array_length(v_ruta, 1), 0) = 0 then
    v_ruta := array['Taller']::text[];
  end if;

  -- Si ya existen trabajos del pedido, se conservan y se vinculan a la OP.
  -- Si no existen, se generan a partir de la ruta.
  ...
  -- Una pieza por unidad solicitada. Esto permite trazabilidad individual.
  v_cantidad := greatest(coalesce(v_pedido.cantidad_piezas, 1), 1);

  if nullif(
       regexp_replace(
         replace(coalesce(v_pedido.peso_estimado, ''), ',', '.'),
         '[^0-9.]',
         '',
         'g'
       ),
  ...
  then
    v_peso := regexp_replace(
      replace(v_pedido.peso_estimado, ',', '.'),
      '[^0-9.]',
      '',
      'g'
    )::numeric;
