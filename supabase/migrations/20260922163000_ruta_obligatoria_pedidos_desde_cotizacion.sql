...
-- Los pedidos convertidos desde cotización/contrato conservan todos sus datos
-- comerciales, pero entran a Pedidos 2 sin una ruta operativa implícita.
-- La ruta se decide en la misma ficha de ingreso que usa un pedido manual.

create or replace function public.limpiar_ruta_inicial_pedido_cotizacion()
returns trigger
language plpgsql
...
begin
  if new.cotizacion_id is not null then
    new.ruta := array[]::text[];
    new.area_actual := 'Pedidos';
  end if;
  return new;
end;
$$;

drop trigger if exists limpiar_ruta_inicial_pedido_cotizacion_trg on public.pedidos;
create trigger limpiar_ruta_inicial_pedido_cotizacion_trg
before insert on public.pedidos
for each row
execute function public.limpiar_ruta_inicial_pedido_cotizacion();

revoke all on function public.limpiar_ruta_inicial_pedido_cotizacion() from public, anon, authenticated;
