...
import { AURUM_IJEWEL_DAROS2_GEMS, getAurumIJEWELDAROS2Reference } from "./aurum/ijewel-daros2-reference";
 ...
 */
export const getAurumIJEWELGemReference=(sourceRootPath:string,materialName?:string)=>
  getAurumIJEWELDAROS2Reference(sourceRootPath,materialName) ?? getAurumIJEWELDAROSReference(sourceRootPath);

export const applyAurumIJEWELReferenceFromMaterial=(material:any,materialName?:string)=>{
  if(!material)return material;
  const ud=material.userData??{};
  const sourceRootPath=String(ud.rootPath??ud.aurumRootPath??ud.WEBGI_rootPath??"");
  const authoredName=String(materialName??ud.name??material.name??"");
  const ref=getAurumIJEWELGemReference(sourceRootPath,authoredName);
...
export { AURUM_IJEWEL_DAROS_GEMS, AURUM_IJEWEL_DAROS2_GEMS };\n\nexport const AURUM_IJEWEL_METAL_REFERENCES = {
  ...
  redGoldLight: { sourceRootPath:"2_metal_redgoldlight_polished_5480463ff7.pmat", baseColorFactor:[0.4392156862745098,0.25882352941176473,0.1607843137254902], roughness:0, ior:1.5, environmentIntensity:0.8 },
  redGold: { sourceRootPath:"2_metal_redgold_polished_448aec7bd1.pmat", baseColorFactor:[0.5058823529411764,0.23529411764705882,0.12156862745098039], roughness:0, ior:1.5, environmentIntensity:0.8 },
  whiteGold: { sourceRootPath:"1_metal_whitegold_polished_0db3fb834b.pmat", baseColorFactor:[0.5394794890033748,0.5394794890033748,0.5457244613615395], roughness:0, ior:1.5, environmentIntensity:1 },
  greenGold: { sourceRootPath:"2_metal_greengold_polished_7c9dfd65ef.pmat", baseColorFactor:[0.5583403896257968,0.4178850708380236,0.17788841597328695], roughness:0, ior:1.5, environmentIntensity:0.8 },
  greyGold: { sourceRootPath:"2_metal_greygold_polished_2b55314ae0.pmat", baseColorFactor:[0.3277780980458375,0.3049873140591091,0.26225065751888765], roughness:0, ior:1.5, environmentIntensity:1 },
  platinum: { sourceRootPath:"2_metal_platinum_polished_9976f87c18.pmat", baseColorFactor:[0.48514994004665124,0.4969329950515914,0.5028864580233624], roughness:0, ior:1.5, environmentIntensity:0.8 },
  silver: { sourceRootPath:"2_metal_silver_polished_fbee436ada.pmat", baseColorFactor:[0.7304607400847158,0.7304607400847158,0.7304607400847158], roughness:0.1, ior:1.5, environmentIntensity:0.8 },
  warmGold: { sourceRootPath:"2_metal_warmgold_polished_c98e843f94.pmat", baseColorFactor:[0.6724431569510133,0.4286904966038916,0.21586050010324417], roughness:0, ior:1.5, environmentIntensity:0.8 },
  yellowGold: { sourceRootPath:"2_metal_yellowgold_polished_b010c8b353.pmat", baseColorFactor:[0.8227857543924378,0.3967552307153359,0.09758734713304495], roughness:0, ior:1.5, environmentIntensity:1 },
  blackRhodium: { sourceRootPath:"2_metal_blackrhodium_polished_1ac9704b86.pmat", baseColorFactor:[0.21568627450980393,0.21568627450980393,0.22745098039215686], roughness:0.03, ior:1.5, environmentIntensity:1, metalness:0.8 },
} as const;
...
export const applyAurumReferenceMetalOptics=(material:any,materialId:string)=>{
  ...
  const key=id.includes("whitegold")||id.includes("oro-blanco") ? "whiteGold"
    : id.includes("redgoldlight") ? "redGoldLight"
    : id.includes("redgold")||id.includes("oro-rojo") ? "redGold"
    : id.includes("rosegold")||id.includes("oro-rosa") ? "roseGold"
    : id.includes("greygold")||id.includes("graygold")||id.includes("oro-gris") ? "greyGold"
    : id.includes("greengold")||id.includes("oro-verde") ? "greenGold"
    : id.includes("platinum")||id.includes("platino") ? "platinum"
    : id.includes("silver")||id.includes("plata") ? "silver"
    : id.includes("warmgold")||id.includes("oro-calido") ? "warmGold"
    : id.includes("yellowgold")||id.includes("oro-amarillo") ? "yellowGold"
    : id.includes("blackrhodium")||id.includes("rodio-negro") ? "blackRhodium"
    : null;
  const ref:any=key?AURUM_IJEWEL_METAL_REFERENCES[key as keyof typeof AURUM_IJEWEL_METAL_REFERENCES]:null;
  if(!ref)return material;
  material.color?.setRGB(ref.baseColorFactor[0],ref.baseColorFactor[1],ref.baseColorFactor[2]);
  material.metalness=ref.metalness??1;
  material.roughness=ref.roughness;
  material.ior=ref.ior;
  material.envMapIntensity=ref.environmentIntensity;
  material.clearcoat=0;
  material.clearcoatRoughness=0;
  material.userData={...(material.userData??{}),aurumIJEWELMetalReference:{...ref,materialId,source:"DAROS (1).glb",renderer:"iJewel/WebGi"},aurumMetalBaseEnvMapIntensity:ref.environmentIntensity};
  material.needsUpdate=true;
  return material;
};

export const getAurumIJEWELMetalReference=(sourceRootPath:string)=>{
  const key=String(sourceRootPath??"").split("/").pop()?.split("?")[0]??"";
  return Object.values(AURUM_IJEWEL_METAL_REFERENCES).find((x:any)=>x.sourceRootPath===key);
};

t * as THREE from "three";
import { AURUM_IJEWEL_DAROS_GEMS, getAurumIJEWELDAROSReference } from "./aurum/ijewel-daros-reference";
