      ...
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <details open className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          ...
          <div className="border-t border-border p-5"><dl className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            ].map(([etiqueta, valor]) => <div key={etiqueta} className="min-w-0 rounded-xl border border-gold/10 bg-gold/[0.025] p-3"><dt className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">{etiqueta}</dt><dd className="mt-1 min-w-0 break-words text-sm font-semibold leading-5">{valor}</dd></div>)}
          </dl></div>
        </details>
        ...
        <details open className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          ...
          <div className="border-t border-border p-5">
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              ].map(([etiqueta, valor]) => <div key={etiqueta} className="rounded-xl border border-gold/10 bg-gold/[0.018] p-4"><p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">{etiqueta}</p><p className="mt-1 break-words text-sm font-semibold leading-5">{valor}</p></div>)}
            </div>
            {pedidoTrabajo?.notas ? <div className="mt-3 rounded-2xl border border-gold/10 bg-gold/[0.018] p-4"><p className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Notas y especificaciones del pedido</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{pedidoTrabajo.notas}</p></div> : null}
          </div>
        </details>
        ...
        <details className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          <summary className="flex cursor-pointer list-none items-center justify-between bg-gold/[0.035] px-5 py-4 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold/10 text-xs font-bold text-gold-deep">6</span><span className="text-sm font-bold">Materiales previstos <span className="ml-1 text-xs text-muted-foreground">({materialesPlanificados.length})</span></span></span><ChevronDown className="size-5 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
          <div className="border-t border-border p-5">{materialesPlanificados.length > 0 ? <div className="space-y-2">{materialesPlanificados.map((item) => <div key={item.id} className="rounded-xl border border-gold/10 bg-gold/[0.018] p-3"><p className="text-sm font-semibold">{item.inventario?.material || "Material"}</p><p className="mt-1 text-xs text-muted-foreground">{item.cantidad_planificada} {item.unidad || item.inventario?.unidad || ""}{item.inventario?.codigo ? " · " + item.inventario.codigo : ""}</p>{item.notas ? <p className="mt-1 text-xs text-muted-foreground">{item.notas}</p> : null}</div>)}</div> : <p className="text-sm text-muted-foreground">No hay materiales planificados para este pedido.</p>}</div>
        </details>

        <details className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          <summary className="flex cursor-pointer list-none items-center justify-between bg-gold/[0.035] px-5 py-4 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold/10 text-xs font-bold text-gold-deep">7</span><span className="text-sm font-bold">Planificación</span></span><ChevronDown className="size-5 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
          <div className="border-t border-border p-5"><dl className="grid grid-cols-2 gap-2 sm:grid-cols-3">{[
          ...
          ].map(([etiqueta, valor]) => <div key={etiqueta} className="rounded-xl border border-gold/10 bg-gold/[0.025] p-3"><dt className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">{etiqueta}</dt><dd className="mt-1 text-sm font-semibold">{valor}</dd></div>)}</dl></div>
        </details>

        {trabajo.area === "Corte Láser" && (pedidoTrabajo?.corte_texto || pedidoTrabajo?.corte_tipografia || pedidoTrabajo?.corte_ubicacion || pedidoTrabajo?.corte_observaciones) ? <details open className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          <summary className="flex cursor-pointer list-none items-center justify-between bg-gold/[0.035] px-5 py-4 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold/10 text-xs font-bold text-gold-deep">8</span><span className="text-sm font-bold">Especificaciones especiales · Corte Láser</span></span><ChevronDown className="size-5 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
        ...
        <details className="group overflow-hidden rounded-2xl border border-gold/25 bg-card shadow-raised">
          <summary className="flex cursor-pointer list-none items-center justify-between bg-gold/[0.035] px-5 py-4 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold/10 text-xs font-bold text-gold-deep">9</span><span className="text-sm font-bold">Tiempo de producción</span></span><ChevronDown className="size-5 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
          <div className="border-t border-border p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Tiempo acumulado</p><h2 className="mt-1 text-xl font-semibold">{Math.floor(segundosTotales / 3600)}h {Math.floor((segundosTotales % 3600) / 60)}m</h2></div><span className={sesionActiva ? "rounded-full bg-success/10 px-2.5 py-1 text-[10px] font-semibold text-success" : "rounded-full bg-gold/[0.06] px-2.5 py-1 text-[10px] font-semibold text-muted-foreground"}>{sesionActiva ? "Reloj activo" : "Pausado"}</span></div>{puedeGestionar ? <div className="mt-4 flex flex-wrap gap-2">{trabajo.estado === "en_proceso" && !sesionActiva ? <button type="button" onClick={() => void iniciarReloj()} className="rounded-xl border border-gold/30 bg-gold/10 px-4 py-2.5 text-xs font-semibold text-gold-deep">▶ Iniciar reloj</button> : null}{sesionActiva ? <button type="button" onClick={() => void detenerReloj()} className="rounded-xl border border-warning/30 bg-warning/10 px-4 py-2.5 text-xs font-semibold text-warning">⏸ Pausar reloj</button> : null}</div> : null}</div>
        </details>

        <details className="group overflow-hidden rounded-2xl border border-gold/10 bg-card shadow-raised">
          <summary className="flex cursor-pointer list-none items-center justify-between bg-gold/[0.035] px-5 py-4 [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-3"><span className="grid size-7 place-items-center rounded-full bg-gold/10 text-xs font-bold text-gold-deep">10</span><span className="text-sm font-bold">Incidencias</span></span><ChevronDown className="size-5 text-muted-foreground transition-transform group-open:rotate-180" /></summary>
          <div className="border-t border-border p-5"><div className="space-y-3">{incidencias.map((item) => <div key={item.id} className="rounded-xl border border-gold/10 bg-gold/[0.018] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold">{item.tipo}</p><p className="mt-1 whitespace-pre-wrap text-sm text-muted-foreground">{item.descripcion}</p></div><span className="rounded-full bg-gold/[0.06] px-2.5 py-1 text-[10px] font-semibold uppercase">{item.estado}</span></div>{item.resolucion ? <p className="mt-3 rounded-xl bg-success-soft p-3 text-xs text-success">Resolución: {item.resolucion}</p> : null}</div>)}{incidencias.length === 0 ? <p className="text-sm text-muted-foreground">No hay incidencias registradas.</p> : null}</div>
