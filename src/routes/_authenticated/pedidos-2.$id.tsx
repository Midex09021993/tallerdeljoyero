...
function Pedido2Detalle() {
  ...
  const { data: participantesServicio = [] } = useQuery({ queryKey: ["pedidos-2-participantes-servicio"], enabled: Boolean(sesion?.esAdmin), queryFn: async () => { if (!sesion?.esAdmin) return []; const { data, error } = await supabase.rpc("listar_participantes_servicio"); if (error) throw error; return data ?? []; } });
        ...
        {trabajos.length ? trabajos.map((t) => {
          ...
          return (
            <div key={t.id} className="flex flex-col gap-3 rounded-xl border border-border bg-surface-sunken p-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-[280px]"><label className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground">Responsable de ejecución</label>{puedeAsignarResponsable ? <><select value={t.participante_id ? "externo" : "interno"} disabled={asignandoTrabajoId === t.id} onChange={(e) => { if (e.target.value === "interno") void asignarParticipanteExterno(t.id, null); else if (!t.participante_id) void asignarParticipanteExterno(t.id, participantesServicio.find((p) => p.especialidad?.trim().toLowerCase() === String(t.area || "").trim().toLowerCase())?.id ?? null); }} className="min-h-9 rounded-xl border border-border bg-card px-3 text-sm disabled:opacity-60"><option value="interno">Taller propio</option><option value="externo">Servicio externo</option></select>{t.participante_id ? <select value={t.participante_id} disabled={asignandoTrabajoId === t.id} onChange={(e) => void asignarParticipanteExterno(t.id, e.target.value || null)} className="min-h-10 rounded-xl border border-border bg-card px-3 text-sm disabled:opacity-60"><option value="">Seleccionar servicio externo</option>{participantesServicio.filter((p) => !p.especialidad || p.especialidad.trim().toLowerCase() === String(t.area || "").trim().toLowerCase()).map((p) => <option key={p.id} value={p.id}>{p.nombre} · {p.especialidad || "Servicio"}</option>)}</select> : <select value={t.responsable_user_id ?? ""} disabled={asignandoTrabajoId === t.id} onChange={(e) => void asignarResponsable(t.id, e.target.value || null)} className="min-h-10 rounded-xl border border-border bg-card px-3 text-sm disabled:opacity-60"><option value="">Sin asignar</option>{operariosDelArea.map((operario) => <option key={operario.id} value={operario.id}>{operario.nombre}</option>)}</select>}{!t.participante_id && operariosDelArea.length === 0 ? <p className="text-[10px] text-warning">No hay operarios internos activos para {t.area}. Puedes enviar el servicio a un participante externo.</p> : null}{t.participante_id ? <p className="text-[10px] text-muted-foreground">El taller externo recibirá y repartirá internamente este servicio.</p> : null}</> : <p className="rounded-xl bg-surface-muted px-3 py-2.5 text-sm font-semibold">{t.participante_id ? "Servicio externo asignado" : t.responsable_user_id ? "Operario asignado" : "Sin asignar"}</p>}</div>/div>
            </div>
          );
