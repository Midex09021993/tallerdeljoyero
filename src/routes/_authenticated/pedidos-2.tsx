...
import { AlertTriangle, ArrowRight, CalendarClock, ClipboardList, Factory, PackageCheck, Plus, Search, UserRound } from "lucide-react";
import { AppShell } from "@/components/AppShell";
import { SelectorSedeDueno, useSedeFiltroDueno } from "@/hooks/use-sede-filtro-dueno";
import { usePedidos, esEstadoFinalPedido, pedidoPendienteAutorizacionProduccion, estadoClases } from "@/lib/taller-db";
  ...
  return (
    ...
    >
      ...
      <section className="mt-6 overflow-hidden rounded-[24px] border border-border bg-card shadow-card">
        ...
        <div className="divide-y divide-border lg:hidden">{lista.map((p) => <button key={p.id} type="button" onClick={() => navigate({ to: "/pedidos/$id", params: { id: p.id }, search: { from: "pedidos" } })} className="w-full p-4 text-left hover:bg-surface-muted/60"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">{p.referencia}</p><p className="mt-1 text-xs text-muted-foreground">{p.cliente || "Cliente pendiente"}</p></div><Status estado={p.estado} /></div><p className="mt-3 text-sm">{p.trabajo || p.pieza || "Sin descripción"}</p><div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground"><span>{p.area_actual || "Sin ubicación"}</span><span>{fmtFecha(p.fecha_entrega ?? p.entrega) || "Sin fecha"}</span></div></button>)}</div>
