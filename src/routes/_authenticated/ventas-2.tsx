...
function Ventas2Page() {
  ...
  const base = vista === "cobros" ? pedidosVenta.filter((p) => p.saldo > 0 || !p.contrato_id) : vista === "despacho" ? [...porEntregar, ...enCamino] : vista === "entregados" ? entregados : activos;
      ...
      <section className="mt-6 rounded-[24px] border border-border bg-card shadow-card">
        ...
        <div className="hidden overflow-x-auto lg:block">
          <table className="w-full text-left">
            <thead className="border-b border-border bg-surface-muted/60"><tr>{["Pedido","Cliente","Venta","Estado","Entrega","Saldo","Acción"].map((h)=><th key={h} className="px-5 py-3 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-border">{lista.map((p)=><VentaRow key={p.id} pedido={p} onOpen={()=>navigate({to:"/ventas-2/$id",params:{id:p.id}})}/>)}</tbody>
          </table>
        </div>
