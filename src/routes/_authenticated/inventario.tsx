...
function InventarioPage() {
  ...
  async function cargar() {
    ...
    const joyasNormalizadas = normalizarCodigosJoyas(joyasCargadas, sesion?.sede?.nombre);

    if (joyasNormalizadas.cambios.length > 0) {
      const resultados = await Promise.all(
        joyasNormalizadas.cambios.map(({ id, codigo }) =>
    ...
    setJoyas(joyasNormalizadas.joyas);
    setCargando(false);
  }
...
function siguienteCodigoJoya(nombreSede: string | null | undefined, joyas: Joya[]) {
  ...
  const usados = new Set(
    joyas
      .map((j) => j.codigo?.match(new RegExp(`^J-${prefijo}-(\\d{3})import { useEffect, useMemo, useState, type FormEvent, type ReactNode } from "react";
import { Link, createFileRoute } from "@tanstack/react-router";
import {
  ArrowDownLeft,
  ArrowUpRight,
...
function Field({ label, value, onChange, type = "text", required = false, select = false, options = [], optionValues = [], step, inputMode }: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean; select?: boolean; options?: string[]; optionValues?: string[]; step?: string; inputMode?: "none" | "text" | "tel" | "url" | "email" | "numeric" | "decimal" | "search" }) {
  ...
  let siguiente = 1;
  while (usados.has(String(siguiente).padStart(3, "0"))) siguiente += 1;
...
function Field({ label, value, onChange, type = "text", required = false, select = false, options = [], optionValues = [], step, inputMode }: { label: string; value: string; onChange: (v: string) => void; type?: string; required?: boolean; select?: boolean; options?: string[]; optionValues?: string[]; step?: string; inputMode?: "none" | "text" | "tel" | "url" | "email" | "numeric" | "decimal" | "search" }) {
  ...
  const resultado = joyas.map((joya) => {
    ...
    usados.add(String(siguiente).padStart(3, "0"));
    siguiente += 1;
