    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <AurumActionCard icon={Plus} title="Nueva cotización" text="Empieza con cliente, pieza y precio." action="Abrir" onClick={() => setAbierto(true)} />
          <AurumActionCard icon={WandSparkles} title="Cuéntaselo al asistente" text="Escribe lo que pidió el cliente y prepara un borrador." action="Abrir" onClick={() => setModoAsistido(true)} />
          <AurumActionCard
            icon={WalletCards}
            title="Aprobadas"
            text={totalAprobadas > 0 ? "Total aprobado: " + money(totalAprobadas) : "Todavía no hay ventas aprobadas."}
            action="Abrir"
            onClick={() => setBusca("aprobada")}
          />
        </div>
