      ...
      <section className="mt-6 overflow-hidden rounded-[24px] border border-border bg-card shadow-card">
        ...
        <div className="hidden overflow-x-auto lg:block">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-border">{lista.map((p) => <PedidoRow key={p.id} pedido={p} esDueno={Boolean(sesion?.esDueno)} onOpen={() => navigate({ to: "/pedidos/$id", params: { id: p.id } })} onDelete={() => setPorBorrar({ id: p.id, referencia: p.referencia })} />)}</tbody>
          </table>
        </div>
      ...
      {sesion?.esAdmin && !isLoading && pedidos.length === 0 ? (
        <section className="mt-4 rounded-2xl border border-warning/30 bg-warning-soft/30 p-4 text-xs">
          ...
          <div className="mt-2 grid gap-1 text-muted-foreground sm:grid-cols-2">
            <p><strong>Auth UID:</strong> {sesion.user.id}</p>
            <p><strong>Profile ID:</strong> {sesion.perfil.id}</p>
            <p><strong>Usuario:</strong> {sesion.perfil.usuario || "vacío"}</p>
            <p><strong>Rol:</strong> {sesion.rolPrincipal}</p>
            <p><strong>Perfil sede_id:</strong> {sesion.perfil.sede_id || "vacío"}</p>
            <p><strong>Sede:</strong> {sesion.sede?.nombre || "vacía"}</p>
            <p><strong>esAdmin:</strong> {String(sesion.esAdmin)}</p>
            <p><strong>esDueno:</strong> {String(sesion.esDueno)}</p>
          </div>
        </section>
      ) : null}
